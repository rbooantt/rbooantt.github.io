...
export default function PredictionForm({ onPrediction, onLoading }) {
  ...
  useEffect(() => {
    ...
    setFormData(prev => ({
      ...prev,
      zona: distritoZonas[formData.distrito][0]
    }));
  }, [formData.distrito]);
  ...
  const handleSubmit = async e => {
    ...
    const payload = {
      ...formData,
      piscina: Number(formData.piscina),
      terraza: Number(formData.terraza),
      jardin: Number(formData.jardin),
      garaje: Number(formData.garaje),
      trastero: Number(formData.trastero),
      calefaccion: Number(formData.calefaccion),
      aire_acondicionado: Number(formData.aire_acondicionado),
      ascensor: Number(formData.ascensor),
      superficie_construida: Number(formData.superficie_construida),
      habitaciones: Number(formData.habitaciones),
      baños: Number(formData.baños)
    };
    ...
    >
        ...
        <select
          ...
          onChange={e => {
            ...
            setFormData(prev => ({
              ...prev,
              distrito: d,
              zona: distritoZonas[d][0]
            }));
            setZonasOptions(distritoZonas[d]);
          }}
          className="w-full border rounded-xl px-4 py-3 text-lg"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
        {features.map(f => (
          <div
            key={f.name}
            className="flex justify-between items-center bg-gray-100 rounded-xl px-4 py-3 cursor-pointer"
            onClick={() =>
              setFormData(prev => ({
                ...prev,
                [f.name]: prev[f.name] === "0" ? "1" : "0"
              }))
            }
          >
            <span>{f.label}</span>
      ...
      <button
        type="submit"
        className="w-full bg-primary text-white font-bold text-lg py-4 rounded-2xl mt-8"
      >
        Predecir Precio
      </button>
    </form>
